...
const WorkspaceCard = ({ workspace }: WorkspaceCardProps) => {
  const location = getLocation(workspace.locationId);
      ...
      <div className="workspace-card overflow-hidden">
        <div className="relative h-48 w-full overflow-hidden">
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-4">
            <p className="text-lg font-semibold text-white">R${workspace.pricePerHour}/hr</p>
          </div>
          <div className="absolute top-3 right-3 bg-white/90 backdrop-blur-sm rounded-full p-2">
            <WorkspaceCategoryIcon category={workspace.category} className="h-4 w-4 text-text-primary" />
          </div>
        </div>
        <div className="p-4">
          ...
          <p className="mt-2 text-sm text-text-muted line-clamp-2">{workspace.description}</p>
          ...
          <div className="mt-3 flex items-center justify-between">
            <span className="text-sm font-medium">Capacidade: {workspace.capacity}</span>
